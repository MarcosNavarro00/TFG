...
def preparacion():
    ...
    data2 = data[['Año', 'Numero de Siniestros']]
...
def predicciones(data_scaled,model,data):
    ...
    data_aux = data[['Año', 'Numero de Siniestros']]
...
def visualizacion():
    ...
    plt.plot(pd.to_datetime(data.index, format='%Y'), data['Numero de Siniestros'], label='Datos Originales', marker='o')
    plt.plot(pd.to_datetime(proximos_años, format='%Y'), predictions, label='Predicciones', marker='o')
    plt.xlabel('Año')
...
def exportar_a_excel(proximos_años,predictions,data_aux):
    ...
    #data_aux.to_excel('predicciones_Siniestros.xlsx')   
